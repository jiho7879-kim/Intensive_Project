...
# 1. 터미널 출력과 파일 출력을 동시에 수행하는 클래스 --> 디버깅용
class Logger:
    """
    터미널 + 파일 동시 출력 로거.
    Windows cp949/euc-kr 환경에서 유니코드(═, Step, ─ 등) 출력 시
...
# 가상환경 설정 오류 때문에 sklearn의 HistGBM으로 대체하는 임시 코드 작성
...
def load_data():
    ...
    # C: EDA + 피처 엔지니어링 --> 이번 프로젝트의 Baseline 피처셋
...
def section1(df, le, FS):
    ...
    print(f"  t-test: t={t_s:.2f}, p={t_p:.2e}")
    ...
    # 상관계수가 0.7 이상인 다중공선성 추정 피처 쌍 찾기
    high_corr = [(PROCESS_FEATURES[i],PROCESS_FEATURES[j],float(corr_m.iloc[i,j]))
                 for i in range(len(PROCESS_FEATURES))
                 for j in range(i+1,len(PROCESS_FEATURES)) if abs(corr_m.iloc[i,j])>0.7]
    # IQR 방식을 사용해서 피처별 이상치 비율 계산
    # Leakage current는 log를 취한후 이상치 계산하는것이 더 좋을듯
    ...
    fig.suptitle('Step1. EDA 심층 분석 — 피처셋 설계 근거',fontsize=16,fontweight='bold',y=1.01)
    ...
    # out_s=pd.Series(outlier_pct).sort_values(ascending=False) #원래 컬럼 기준으로
    out_s=pd.Series(outlier_pct_log).sort_values(ascending=False) # leakage log취한거 기준으로
    ax.barh(out_s.index,out_s.values,
            color=['#e74c3c' if v>1 else '#f39c12' if v>0.5 else '#2ecc71' for v in out_s.values],alpha=0.85)
    ax.axvline(1,color='red',linestyle='--',lw=1,label='1% 기준')
    ax.set_xlabel('이상치(%)'); ax.set_title('D. 피처별 이상치 비율',fontweight='bold'); ax.legend(fontsize=8)

    # 피처셋별 R2 성능비교
    ax=fig.add_subplot(gs[1,1])
    fl=['A','B','C★']; fv=[fs_r2[k] for k in ['A','B','C']]
    bars=ax.bar(fl,fv,color=['#95a5a6','#3498db','#e74c3c'],alpha=0.85)
...
# ══════════════════════════════════════════════════════════════
# Step2  Phase 1 — 26개 모델 베이스라인
# ══════════════════════════════════════════════════════════════
def section2(df, FS):
    banner(f"Step2  Phase 1 — Baseline model screening (26개 모델)")
    ...
    #     'SVR (linear)':          (SVR(kernel='linear',C=1.0),                                True,  'linear'),
        
    #     # ── 이상치 강건 (5) ────────────────────────────────────
    #     # Robust Regression 참고 https://velog.io/@watermelon870/Robust-Regression 
    ...
    #     'RANSAC + Ridge':       (ransac,                                                  True,  'robust'), #ransac 단독으로 사용시 오류 발생으로 Ridge와 같이 사용하는 방식으로 수정
    #     'Bayesian Ridge':       (BayesianRidge(),                                         True,  'robust'), #https://torahee.tistory.com/160
    #     'Kernel Ridge (RBF)':   (KernelRidge(alpha=1.0,kernel='rbf'),                     True,  'robust'), #https://direction-f.tistory.com/89 
        
        
    #     # ── 비선형 (4) ────────────────────────────────
    #     'SVR (RBF)':           (SVR(kernel='rbf',C=1.0),                                 True,  'nonlinear'),
    #     'SVR (POLY)':          (SVR(kernel='poly',C=1.0),                                True,  'nonlinear'),
    #     'NuSVR':               (NuSVR(kernel='rbf',nu=0.5),                              True,  'nonlinear'), #매개변수 Nu를 통해 오차 튜닝어 좋다고 알려짐
    ...
    #260611 updated ; xai 과정에서 permutation importance 추출시 scale여부가 문제가 될 수 있어 pipe단계에서 scale하는것으로 model 구조 변경
    MODELS = {
        # ── 선형 (7) ───────────────────────────────────────────
        'Linear Regression':    (pipe(LinearRegression()),                                        'linear'),
        'Ridge':                (pipe(Ridge(alpha=1.0)),                                          'linear'),
        'Lasso':                (pipe(Lasso(alpha=0.001,max_iter=5000)),                          'linear'),
        'ElasticNet':           (pipe(ElasticNet(alpha=0.001,l1_ratio=0.5,max_iter=5000)),        'linear'),
        'PLS (n=8)':           (pipe(PLSRegression(n_components=8)),                             'linear'),
        'PCR (PCA+Ridge)':     (make_pipeline(StandardScaler(), PCA(n_components=0.95), Ridge()),  'linear'),
        'SVR (linear)':          (pipe(SVR(kernel='linear',C=1.0)),                                  'linear'),
        
        # ── 이상치 강건 (5) ────────────────────────────────────
        # Robust Regression 참고 https://velog.io/@watermelon870/Robust-Regression 
        'Huber Regressor':      (pipe(HuberRegressor(epsilon=1.35,max_iter=300)),                 'robust'),
        'TheilSen':             (pipe(TheilSenRegressor(random_state=RANDOM_STATE,
                                                   max_subpopulation=500)),                  'robust'),
        'RANSAC + Ridge':       (pipe(ransac),                                                    'robust'), #ransac 단독으로 사용시 오류 발생으로 Ridge와 같이 사용하는 방식으로 수정
        'Bayesian Ridge':       (pipe(BayesianRidge()),                                           'robust'), #https://torahee.tistory.com/160
        'Kernel Ridge (RBF)':   (pipe(KernelRidge(alpha=1.0,kernel='rbf')),                       'robust'), #https://direction-f.tistory.com/89 
        
        
        # ── 비선형 (4) ────────────────────────────────
        'SVR (RBF)':           (pipe(SVR(kernel='rbf',C=1.0)),                                   'nonlinear'),
        'SVR (POLY)':          (pipe(SVR(kernel='poly',C=1.0)),                                  'nonlinear'),
        'NuSVR':               (pipe(NuSVR(kernel='rbf',nu=0.5)),                                'nonlinear'), #매개변수 Nu를 통해 오차 튜닝어 좋다고 알려짐
        'KNN (k=5)':           (pipe(KNeighborsRegressor(n_neighbors=5)),                        'nonlinear'),
        
        # ── 단일 트리 (1) ──────────────────────────────────────
        'Decision Tree':        (pipe(DecisionTreeRegressor(max_depth=8,
                                                       random_state=RANDOM_STATE)),        'tree'),
        
        # ── 앙상블 (4) ─────────────────────────────────────────
        'Bagging (DT)':         (pipe(BaggingRegressor(
                                    estimator=DecisionTreeRegressor(max_depth=6),
                                    n_estimators=50,random_state=RANDOM_STATE,
                                    n_jobs=-1)),                                            'ensemble'),
        'Random Forest':        (pipe(RandomForestRegressor(n_estimators=100,
                                    random_state=RANDOM_STATE,n_jobs=-1)),                  'ensemble'),
        'Extra Trees':          (pipe(ExtraTreesRegressor(n_estimators=100,
                                    random_state=RANDOM_STATE,n_jobs=-1)),                  'ensemble'),
        'AdaBoost':             (pipe(AdaBoostRegressor(n_estimators=100,
                                    random_state=RANDOM_STATE)),                            'ensemble'),
        
        # ── 부스팅 (5) ─────────────────────────────────────────
        'Gradient Boosting':    (pipe(GradientBoostingRegressor(n_estimators=100,
                                    random_state=RANDOM_STATE)),                            'boosting'),
        'HistGradientBoost':    (pipe(HistGradientBoostingRegressor(random_state=RANDOM_STATE)),  'boosting'),
        xgb_name:               (pipe(BOOST_LIBS['xgb']),                                        'boosting'),
        lgb_name:               (pipe(BOOST_LIBS['lgb']),                                        'boosting'),
        cb_name:                (pipe(BOOST_LIBS['cb'])         ,                                         'boosting'),
    }
    ...
    for name,(model,cat) in MODELS.items():
        try:
            ...
            print(f"  ⚠️ {name} 실패: {e}")
            r2m=r2s=rmse=mae=0.0
        p1[name]=dict(R2_mean=r2m,R2_std=r2s,RMSE=rmse,MAE=mae,category=cat)
        print(f"  {name:<30s}  {r2m:>8.4f}  {r2s:>7.4f}  {rmse:>7.4f}  {cat:>10s}")
    ...
    fig.suptitle(f'Step2. Phase 1 — Baseline model screening ({len(MODELS)}개 모델, {CV_FOLDS}-Fold CV)',
...
# ══════════════════════════════════════════════════════════════
# Step3  Phase 2 — 하이퍼파라미터 튜닝 후 Best model 선정
# ══════════════════════════════════════════════════════════════
def section3(df, FS, p1_results):
    banner("Step3  Phase 2 — 하이퍼파라미터 튜닝 (RandomizedSearchCV)")
    X=df[FS['C']].values; y=df[TARGET].values
    ...
    def mp(params): #search_spaces의 parameter key를 model__ prefix에 일괄 wrapping하는 함수
    ...
    #     },
    # }
    # # XGB/LGB/CB 패키지 없는 경우 HistGM으로 Search 진행
    ...
    # pipeline에 scaling적용으로 변경
    SEARCH_SPACES={
        'Gradient Boosting':{
            'model': pipe(GradientBoostingRegressor(random_state=RANDOM_STATE)),
            'params': mp({'n_estimators':[100,200,300],'learning_rate':[0.01,0.03,0.05,0.08,0.1,0.15],
                          'max_depth':[2,3,4,5],'subsample':[0.7,0.8,0.9,1.0],
                          'min_samples_leaf':[1,2,4],'max_features':['sqrt',0.7,0.9,None]}),
            'n_iter':40},
        'Extra Trees': {
            'model': pipe(ExtraTreesRegressor(random_state=RANDOM_STATE, n_jobs=-1)),
            'params': mp({'n_estimators': [100, 200, 300, 500],
                'max_depth': [5, 8, 12, None],
                'min_samples_split': [2, 5, 10],
                'min_samples_leaf': [1, 2, 4],
                'max_features': ['sqrt', 'log2', 0.5, 0.7],
            ...
            'n_iter': 25},
        'HistGradientBoost':{
            'model': pipe(HistGradientBoostingRegressor(random_state=RANDOM_STATE)),
            'params': mp({'max_iter': [100, 200, 300],
                'learning_rate': [0.01, 0.05, 0.1, 0.15],
                'max_depth': [3, 5, 7, None],
                'min_samples_leaf': [10, 20, 30, 50],
                'l2_regularization': [0.0, 0.01, 0.1, 1.0],
    ...
    for key,name,extra_p in [
        ...
        if name in p1_results:
            model_inst = BOOST_LIBS[key]
            if 'HistGBM' in name or '대안' in name:
                SEARCH_SPACES[name]={'model': pipe(HistGradientBoostingRegressor(random_state=RANDOM_STATE)),
                                     'params': mp(extra_p), 'n_iter':25}

    if 'Random Forest' in p1_results:
        SEARCH_SPACES['Random Forest']={
            'model': pipe(RandomForestRegressor(random_state=RANDOM_STATE,n_jobs=-1)),
            'params': mp({'n_estimators':[100,200,300,500],'max_depth':[5,8,12,None],
                          'min_samples_split':[2,5,10],'min_samples_leaf':[1,2,4],
                          'max_features':['sqrt','log2',0.5,0.7],'bootstrap':[True,False]}),
            'n_iter':40}
    if 'AdaBoost' in p1_results:
        SEARCH_SPACES['AdaBoost']={
            'model': pipe(AdaBoostRegressor(random_state=RANDOM_STATE)),
            'params': mp({'n_estimators':[100,200,300,500],'learning_rate':[0.01,0.05,0.1,0.5,1.0],
                          'loss':['linear','square','exponential']}),
            'n_iter':25}
    ...
    for name,cfg in SEARCH_SPACES.items():
        ...
        print(f"\n  [{name}] Baseline={bl:.4f} → Search (n_iter={cfg['n_iter']})...",end=' ',flush=True)
        t0=time.time()
        ...
        tuning_results[name]=dict(baseline_r2=bl,search_best_r2=float(search.best_score_),
            ...
            best_params=clean_params,elapsed=elapsed)
    ...
    fig.suptitle('Step3. Phase 2 — 하이퍼파라미터 튜닝 결과',fontsize=13,fontweight='bold')
    ax=axes[0]
    ...
    # fig.suptitle('Step3. Learning Curve — 상위 5개 모델',fontsize=13,fontweight='bold')
    ...
    for i, ax_ in enumerate(axes):
        if i < len(top3): # 모델이 있는 경우에만 그림
            ...
            gap=tr_m[-1]-va_m[-1]
            ax_.text(0.05,0.07,f'Overfitting Gap={gap:.3f}',transform=ax_.transAxes,fontsize=8,
                    bbox=dict(boxstyle='round',facecolor='white',alpha=0.8))
    save_fig(fig,'sec3b_learning_curve')
    return tuning_results, best_estimators

def section3_1(df, FS, p1_results): #중간에 추가해서 step11로 동작
    banner("Step3-1. Phase 2 — 26개 모든 모델 하이퍼파라미터 통합 튜닝 및 성능 비교")
    X = df[FS['C']].values
    y = df[TARGET].values
    
    # 스케일링이 필요한 모델과 아닌 모델을 구분하기 위한 유틸리티 함수 -> MODELS딕셔너리에 True/False로 집적 지정 가능
    ...
    # 모든 모델을 한번에 진행하기 위한 파이프라인에 사용할 딕셔너리
    # 모델이름 : get_pipe(모델객체, 스케일링여부), 카테고리
    MODELS = {
        ...
        'SVR (linear)': (get_pipe(SVR(kernel='linear', C=1.0)), 'linear'),
        ...
        'Decision Tree': (get_pipe(DecisionTreeRegressor(max_depth=8, random_state=RANDOM_STATE)), 'tree'),
        
        'Bagging (DT)': (get_pipe(BaggingRegressor(estimator=DecisionTreeRegressor(max_depth=6), n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1)), 'ensemble'),
        'Random Forest': (get_pipe(RandomForestRegressor(n_estimators=100, random_state=RANDOM_STATE, n_jobs=-1)), 'ensemble'),
        'Extra Trees': (get_pipe(ExtraTreesRegressor(n_estimators=100, random_state=RANDOM_STATE, n_jobs=-1)), 'ensemble'),
        'AdaBoost': (get_pipe(AdaBoostRegressor(n_estimators=100, random_state=RANDOM_STATE)), 'ensemble'),
        'Gradient Boosting': (get_pipe(GradientBoostingRegressor(n_estimators=100, random_state=RANDOM_STATE)), 'boosting'),
        'HistGradientBoost': (get_pipe(HistGradientBoostingRegressor(random_state=RANDOM_STATE)), 'boosting'),
        xgb_name: (get_pipe(BOOST_LIBS['xgb']), 'boosting'),
        lgb_name: (get_pipe(BOOST_LIBS['lgb']), 'boosting'),
        cb_name: (get_pipe(BOOST_LIBS['cb']), 'boosting'),
    }
    ...
    SEARCH_SPACES = {
        ...
        for name, p_cfg in {
            ...
            'SVR (linear)': {'params': {'C': [0.1, 1.0, 10], 'gamma': ['scale', 'auto']}, 'n_iter': 6},
    ...
    # PCR은 파라미터 이름이 달라서 예외적으로 딕셔너리 후속 처리
    ...
    fig.suptitle('Step3-1. 모델 튜닝 성능 및 단계별 비교 분석', fontsize=15, fontweight='bold')
...
def section4(df, FS, tuning_results, best_estimators):
    ...
    fig.suptitle('Step4. 전체 모델 Leaderboard',fontsize=14,fontweight='bold')
    bar_colors=[]
    for m in lb['model']:
...
def section5_6_7(df, FS, tuning_results, best_estimators, adv_results):
    banner("Step5–7  최종 모델 평가 / EDA 비교 / 종합 요약")
    ...
    FINAL.metadata = tuning_results
    FINAL.best_name = best_name
    ...
    fig.suptitle(f'Step5. 최종 모델 평가 — {best_name}',fontsize=14,fontweight='bold')
    ax=axes[0,0]
    for node in nodes_:
        mask=df['technology_node']==node
    ...
    ax.set_xlabel('Predicted'); ax.set_ylabel('Residual'); ax.set_title('B. Residual 분석',fontweight='bold'); ax.legend(fontsize=8)
    ax=axes[0,2]
    ...
    ax.axhline(0,color='red',linestyle='--',lw=1.5); ax.set_ylabel('Residual'); ax.set_title('F. 노드별 Residual',fontweight='bold')
    ...
    actual_model = best_estimators[best_name]
    if hasattr(actual_model, 'named_steps') and 'model' in actual_model.named_steps:
        actual_model = actual_model.named_steps['model']

    # 2. 실제 모델의 타입을 검사하여 새 모델 객체를 생성합니다.
    if isinstance(actual_model, GradientBoostingRegressor):
        cmp_model = GradientBoostingRegressor(**bp_raw, random_state=RANDOM_STATE)
    else:
        # HistGradientBoostingRegressor일 경우 호환되지 않는 파라미터 제거
        invalid_keys = {'subsample', 'n_estimators', 'max_features', 'min_samples_leaf'}
        hgbm_params = {k: v for k, v in bp_raw.items() if k not in invalid_keys}
        cmp_model = HistGradientBoostingRegressor(**hgbm_params, random_state=RANDOM_STATE)
    ...
    for mname,model in COMP.items():
        row={'Model':mname}
        for fsn,fset in [('A-Baseline',FS_A_),('B-다중공선성제거',FS_B_),('C-EDA+파생피처★',FS_C)]:
    ...
    fig.suptitle('Step6. 피처셋 효과 비교',fontsize=13,fontweight='bold')
    ...
    fig.suptitle('Step7. Best Model 선정 — 전 과정 요약',fontsize=15,fontweight='bold',y=1.01)
    gs7=gridspec.GridSpec(1,2,figure=fig,hspace=0.5,wspace=0.4)
    ...
    ax.set_title('A. Phase별 최고 성능 비교',fontweight='bold',fontsize=12)
    ax2=fig.add_subplot(gs7[0,0]); ax2.axis('off')
    param_text='\n'.join([f"  {k}: {v}" for k,v in tuning_results[best_name]['best_params'].items()])
    ax2.text(0.95,.8,f"★ Best: {best_name}\n\n{param_text}\n\n"
...
#     # ── ① Step5에서 22nm R² 낮음 발견 → 노드별 개별 모델 ─────
#     print("\n  [의사결정 ①]")
#     print("  발견: Step5 진단 결과 22nm R²<0.5 — 노드별 성능 격차 심각")
...
#     # ── ③ 피처 표현력 → FS-D 추가 파생 ────────────────────
#     print("\n  [의사결정 ③]")
#     print(f"  발견: FS-C R²={best_r2:.4f} 정체 — 피처 표현력 한계 가능성")
...
#     print(f"    FS-C={r2_c:.4f}  →  FS-D(26개)={r2_d:.4f}  diff={r2_d-r2_c:+.4f}")
...
#     fig.suptitle('Step8. 개선 실험 — 의사결정 흐름', fontsize=14, fontweight='bold', y=1.01)
...
# XAI
# ══════════════════════════════════════════════════════════════
def run_xai(df, FS, FINAL, n_seeds=3):
    banner("XAI — 6-Method Voting 유의인자 추출 (Multi-Seed Averaging)")
    X = df[FS['C']].values; y = df[TARGET].values
    ...
    # 1. GB Built-in (학습된 모델이므로 한 번만 계산)
    inner_model = _inner(FINAL)
    gb_n = norm(inner_model.feature_importances_ if hasattr(inner_model, 'feature_importances_') else np.ones(X.shape[1]))
    
    # 2, 3, 4. Multi-seed 반영 (RF, ET, Permutation)
    for seed in seeds:
        print(f"  [Seed {seed}] 모델 학습 및 중요도 계산 중...", end=' ', flush=True)
        ...
        # Permutation
        perm = permutation_importance(FINAL, X, y, n_repeats=20, random_state=seed, n_jobs=-1)
    ...
    # 평균 산출
    rf_n = rf_accum / n_seeds
    et_n = et_accum / n_seeds
    perm_n = perm_accum / n_seeds
    shap_n = shap_accum / n_seeds

    # 5. Pearson (고정값)
    print("  [5/5] Pearson |r| 계산 완료")
    pear_n = norm(np.array([abs(stats.pearsonr(X[:,i], y)[0]) for i in range(X.shape[1])]))
    
    # 앙상블 
    ens = (gb_n + rf_n + et_n + perm_n + shap_n + pear_n) / 6
    
    # 결과 데이터프레임 구성 (이후 시각화 로직은 동일)
    ...
    hm=xai_df[['GB','RF','ET','Permutation','SHAP','Pearson']].values
    fig,axes=plt.subplots(1,2,figsize=(17,9))
    fig.suptitle('XAI. 6-Method Voting Feature Importance',fontsize=13,fontweight='bold')
    im=axes[0].imshow(hm,aspect='auto',cmap='YlOrRd')
    axes[0].set_xticks(range(6)); axes[0].set_xticklabels(['GB','RF','ET','Perm','SHAP','Pearson'],fontsize=10,fontweight='bold')
...
def main():
    ...
    to_run=sorted(args.steps if args.steps else [1,2,3,11,4,5,7])
    if args.skip: to_run=[s for s in to_run if s not in args.skip]
    desc={1:'Step1 EDA',2:f'Step2 Baseline(26모델)',3:'Step3 Tuning+best_params', 11:'$3-1 모든 model tuning',
          4:'Step4 Ensemble',5:'Step5-7 진단/EDA비교/요약',7:'XAI'}
    print("="*65)
...
# # ══════════════════════════════════════════════════════════════
# # Step9-1  후속 개선 실험 — Step8-① 심층 탐색
# # ══════════════════════════════════════════════════════════════
# def section9_1(df, FS, tuning_results, best_estimators, FINAL):
#     banner("Step9-1  후속 개선 실험 — 전이학습 / 계층 모델 / 노드별 정규화")
#     FS_C = FS['C']
#     X = df[FS_C].values; y = df[TARGET].values
#     best_name = max(tuning_results, key=lambda k: tuning_results[k]['cv5_r2'])
...
#     print("\n  [Step9-1 B] 계층적 모델링 (Level1 Global + Level2 Residual)")
#     print("  설계: Level1(통합 GBM) Residual를 노드별 GBM이 학습, 최종 = L1 + L2")
...
#     print("  판단: ❌ 기각 — Residual σ≈0.1이 랜덤에 가까워 Level2가 노이즈 과적합")

#     # ── C: 노드별 그룹 정규화 + Relative Performance ──────────
#     print("\n  [Step9-1 C] 노드별 그룹 정규화 + Relative Performance 피처")
...
#     ax.text(0.05,0.95,
#         "B. 핵심 원인:\n"
#         "• Residual μ≈-0.0004, σ≈0.0997\n  (거의 랜덤 노이즈)\n"
#         "• Level2가 학습할 체계적 패턴 없음\n"
#         "• SNR(신호대잡음비) < 1\n\n"
#         "계층 모델이 효과적인 조건:\n"
#         "• Level1에 체계적 편향(bias) 존재\n"
#         "• Residual에 노드별 패턴이 명확\n"
#         "→ 현재 데이터에서 미충족",
#         transform=ax.transAxes,fontsize=9,va='top',
...
# # ══════════════════════════════════════════════════════════════
# # Step9-2  후속 개선 실험 — Step8-② 심층 탐색
# # ══════════════════════════════════════════════════════════════
# def section9_2(df, FS, tuning_results, best_estimators, FINAL):
#     banner("Step9-2  후속 개선 실험 — Expert 샘플링 / Residual Stacking / FS 분리")
#     FS_C=FS['C']; X=df[FS_C].values; y=df[TARGET].values
#     best_name=max(tuning_results,key=lambda k:tuning_results[k]['cv5_r2'])
...
#     print("\n  [Step9-2 A] Expert 가중치 샘플링 (Bagging of Specialists)")
...
#     print("\n  [Step9-2 B] Residual 보정형 Stacking (Residual Stacking)")
#     Xs=StandardScaler().fit_transform(X)
...
#     print(f"    SVR Residual R²={r2_svr_res:.4f}  RF Residual R²={r2_rf_res:.4f}")
#     print(f"    GBM+RFResidual 최종: {r2_corr:.4f}  {r2_corr-baseline:+.4f}")
#     print("    판단: ❌ 기각 — Residual SNR 극히 낮음, SVR 역방향 과적합")

#     print("\n  [Step9-2 C] 피처 서브스페이스 전략적 할당")
...
#     fig.suptitle('Step9-2. 후속 개선 실험 — Step8-② 심층 탐색',fontsize=13,fontweight='bold')
#     ax=axes[0]
#     ms=['GBM\nBaseline','Step8 이기종\nStacking','High\nExpert','Low\nExpert','★Expert\nStacking']
...
#     ax2.set_title('B. Residual 보정 Stacking\n❌ 기각',fontweight='bold',color='#c0392b')
...
# # ══════════════════════════════════════════════════════════════
# # Step9-3  후속 개선 실험 — Step8-④ Residual 분석 기반 타겟팅 전략
# # ══════════════════════════════════════════════════════════════
# def section9_3(df, FS, tuning_results, best_estimators, FINAL):
#     banner("Step9-3  후속 개선 실험 — Residual 타겟팅 전략 (A+B+C)")
#     FS_C=FS['C']; X=df[FS_C].values; y=df[TARGET].values
#     best_name=max(tuning_results,key=lambda k:tuning_results[k]['cv5_r2'])
#     baseline=tuning_results[best_name]['cv5_r2']
...
#     # ── A: 하이브리드 샘플링 ─────────────────────────────────
#     print("\n  [Step9-3 A] Residual 타겟팅 하이브리드 샘플링")
...
#     # ── B: Residual 보정 모델 ──────────────────────────────────
#     print("\n  [Step9-3 B] 노드별 Residual 보정 모델 (Error Corrector)")
...
#     print("  판단: ❌ 기각 — Residual 패턴 부재 + 소규모 과적합")

#     # ── C: 그룹 Expert 앙상블 ─────────────────────────────
#     print("\n  [Step9-3 C] 도메인 기반 그룹 Expert 앙상블")
...
#     # ── A+C 결합 ──────────────────────────────────────────
#     print("\n  [Step9-3 A+C 결합] 가중치 샘플링 + 그룹 Expert 앙상블")
#     w_ac=np.ones(len(df))
...
#     print(f"  B Residual 보정 모델:    {r2_B:.4f}  {r2_B-baseline:+.4f}  ❌")
...
#     # Figure
#     fig,axes=plt.subplots(1,3,figsize=(18,6))
#     fig.suptitle('Step9-3. Residual 타겟팅 전략',fontsize=13,fontweight='bold')
...
#     axes[1].set_title('B. Residual 보정 모델\n→ ❌ 기각',fontweight='bold',color='#c0392b')
