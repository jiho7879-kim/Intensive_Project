"""
semiconductor_yield_xai.py  ─ Updated Version
══════════════════════════════════════════════════════════════
반도체 수율 예측 & XAI 유의인자 분석 서비스

변경사항:
  - §8 개선실험 탭 제거
  - 파이프라인 실행 패널 추가 (각 Step 개별 실행)
  - 예측 탭: 예측용 CSV 번들 제공 + Train vs Predict Scatter Plot
  - EDA 히트맵: 하삼각 마스킹 적용
  - EDA: Route Path 시각화 추가 (노드별 공정 경로)

실행:  streamlit run semiconductor_yield_xai.py
══════════════════════════════════════════════════════════════
"""
...
FEATURE_LABELS = {
    'etch_rate':'Etch Rate (nm/min)','pressure':'Chamber Pressure (mTorr)',
    'temperature':'Process Temperature (°C)','exposure_time':'Exposure Time (s)',
    'focus_offset':'Focus Offset (nm)','dose':'Implant Dose (ions/cm²)',
    'deposition_rate':'Deposition Rate (nm/min)',
    'thickness_uniformity':'Thickness Uniformity (%)',
    'implant_energy':'Implant Energy (keV)','tilt_angle':'Tilt Angle (°)',
    'critical_dimension':'Critical Dimension (nm)',
    'oxide_thickness':'Oxide Thickness (Å)','resistivity':'Resistivity (Ω·cm)',
    'defect_count':'Defect Count (#)','defect_density':'Defect Density (def/cm²)',
    'vth':'Threshold Voltage Vth (V)','leakage_current':'Leakage Current (A)',
    'resistance':'Resistance (Ω)','tech_encoded':'Tech Node (encoded)',
    'is_advanced_node':'Advanced Node Flag (7/10nm=1)',
    'defect_composite':'Defect Composite Index (log)',
    'cd_vth_product':'CD × Vth','uniformity_x_defect':'Uniformity × Defect',
    'node_x_cd':'Node × CD',
}
RANDOM_STATE = 42
...
@st.cache_data(show_spinner="XAI 5-Method 분석 중 (약 40초)...")
def run_xai(_model, X, y, feature_names):
    gb_n   = norm(_model.feature_importances_)
    rf     = RandomForestRegressor(n_estimators=200, random_state=RANDOM_STATE, n_jobs=-1)
    rf.fit(X, y); rf_n = norm(rf.feature_importances_)
    et     = ExtraTreesRegressor(n_estimators=200, random_state=RANDOM_STATE, n_jobs=-1)
    et.fit(X, y); et_n = norm(et.feature_importances_)
    perm   = permutation_importance(_model, X, y, n_repeats=10,
                                    random_state=RANDOM_STATE, n_jobs=-1)
    perm_n = norm(np.clip(perm.importances_mean, 0, None))
    pear_n = norm(np.array([abs(stats.pearsonr(X[:,i], y)[0])
...
"""
    ...
    # ── 파이프라인 스텝 정의 ──────────────────────────────────
    STEPS = [
        {"id": 1, "label": "§1 EDA 심층 분석",       "desc": "이분화 검증, 다중공선성, 피처셋 A/B/C/D R² 비교", "time": "~20초", "color": "#22d3ee"},
        {"id": 2, "label": "§2 Baseline 스크리닝",    "desc": "27개 모델 5-Fold CV 평가 (FS-C 기준)",            "time": "~3분",  "color": "#3b82f6"},
        {"id": 3, "label": "§3 Hyper-param Tuning",  "desc": "Top5 RandomizedSearchCV + best_params 저장",     "time": "~5분",  "color": "#8b5cf6"},
        {"id": 4, "label": "§4 Ensemble",             "desc": "Stacking / Voting (동종 Top3)",                  "time": "~2분",  "color": "#f59e0b"},
        {"id": 5, "label": "§5-7 진단·EDA비교·요약", "desc": "잔차, Q-Q, 노드별 R², Best model 선정 종합",      "time": "~3분",  "color": "#10b981"},
        {"id": 6, "label": "§8 개선 실험",            "desc": "노드별 개별·이기종 앙상블·FS-D·커널 규명",        "time": "~5분",  "color": "#ef4444"},
        {"id": 7, "label": "XAI 유의인자",            "desc": "5-Method Voting (GB/RF/ET/Perm/Pearson)",        "time": "~2분",  "color": "#ec4899"},
    ]
    ...
    with col_all2:
        run_quick = st.button("⚡ 핵심만 (§1~§5+XAI)", use_container_width=True,
    ...
    if run_all and can_run:
        with st.expander("▼ 전체 파이프라인 실행 로그", expanded=True):
            run_pipeline_step([1,2,3,4,5,6,7], "전체 파이프라인")
        st.rerun()
    if run_quick and can_run:
        with st.expander("▼ 핵심 파이프라인 실행 로그", expanded=True):
            run_pipeline_step([1,2,3,4,5,7], "핵심 파이프라인 (§1~§5+XAI)")
        st.rerun()
    ...
    for step in STEPS:
        ...
        with col4:
            btn_key = f"run_step_{sid}"
            if st.button(f"▶ Step {sid}", key=btn_key,
                         use_container_width=True, disabled=not can_run):
                with st.expander(f"▼ Step {sid} — {step['label']} 실행 로그", expanded=True):
    ...
    if not results and not figures:
        ...
        fig_tabs_labels = {
            'sec1_eda_deep':         '§1 EDA',
            'sec2_phase1_screening': '§2 Baseline',
            'sec3a_tuning_comparison':'§3 Tuning',
            'sec4_leaderboard':      '§4 Leaderboard',
            'sec5_final_diagnostics':'§5 진단',
            'sec7_final_summary':    '§7 요약',
            'xai_voting':            'XAI',
        }
...
with tabs[1]:
    ...
    if len(hg)>0 and len(lg)>0:
        ...
        insight(f'💡 <b>이분화 검증</b> — 고수율(7/10nm) μ={hg.mean():.3f} vs '
    ...
    # ─── 피처 상관행렬 히트맵 (하삼각 마스킹) ───────────────────
    st.markdown("---")
    st.subheader("피처 상관행렬 히트맵 (하삼각 표시)")
    ...
    ax.set_title(f'상관행렬 — 하삼각 ({node_sel})', fontsize=12, fontweight='bold')
    plt.tight_layout(); st.pyplot(fig); plt.close()

    # ─── Route Path 시각화 ─────────────────────────────────────
    st.markdown("---")
    st.subheader("🗺️ 공정 Route Path 분석 (노드별 Tool 조합 경로)")
    insight("💡 각 Wafer가 통과한 4개 Tool의 조합(Route)을 노드별로 시각화합니다. "
            "단일 경로 집중도가 높을수록 공정 표준화가 잘 된 노드입니다.", 'blue')
    ...
    with route_tab2:
        ...
        insight(f"📌 <b>{route_node} Route 분석</b>: "
                ...
                f"평균 Yield 최고 경로: <b>{route_yield.loc[route_yield['mean_yield'].idxmax(),'route']}</b> "
                f"(μY={route_yield['mean_yield'].max():.3f})", 'green')
...
with tabs[2]:
    ...
    st.markdown("""
**EDA 반영 피처셋 C (23개) 설계 근거:**
| 조치 | 내용 | 근거 |
|---|---|---|
| **제거** | `defect_density` | thickness_uniformity와 r=0.9996 (사실상 중복) |
| **추가①** | `is_advanced_node` | 7/10nm 이분화 명시 (t=35.1, p<10⁻¹⁸⁸) |
| **추가②** | `defect_composite` | log(defect_count × defect_density) — 복합 결함 지수 |
| **추가③** | `cd_vth_product` | critical_dimension × vth — 공정-전기 상호작용 |
| **추가④** | `uniformity_x_defect` | thickness_uniformity × defect_count |
| **추가⑤** | `node_x_cd` | tech_encoded × critical_dimension — 노드×CD 상호작용 |
""")

    st.markdown("---")
    st.subheader("피처셋 A / B / C / D — R² 비교")
    ...
    fs_cfg = [
        ('A — Baseline (19개)',       FS_A,  '#95a5a6'),
        ('B — EDA 다중공선성 제거 (18개)', FS_B,  '#3498db'),
        ('C — EDA+FE (23개) ★',      FS_C,  '#e74c3c'),
        ('D — 추가 파생 (26개)',       FS_D,  '#e67e22'),
    ]
    fs_r2 = {}
    ...
    with col_fs2:
        insight('✅ <b>피처셋 C 채택 근거</b><br>EDA+FE로 Baseline 대비 R² 개선. '
                '<code>node_x_cd</code>(노드×CD 상호작용), '
                '<code>is_advanced_node</code>(이분화 플래그)가 핵심 기여. '
                'D(추가파생)는 C와 동등 → GBM이 비선형성을 내부적으로 포착하므로 '
                '단순 수식 변환의 한계 효용 낮음.', 'green')
        insight('💡 <b>다중공선성 처리</b><br>'
                'defect_density vs thickness_uniformity: r=0.9996. '
                '트리 모델은 다중공선성에 강하나 <code>defect_composite</code>로 '
                '정보 손실 없이 대체.', 'blue')
    ...
    st.markdown("---")
    st.subheader("Tool Group 내 피처 기여도")
...
with tabs[3]:
    ...
    st.markdown("""
**모델 선정 3-Phase 요약:**
- **Phase 1**: 27개 모델 베이스라인 스크리닝 (EDA 피처셋 C 기준)
- **Phase 2**: Top 5 RandomizedSearchCV 튜닝 — 각 파라미터 설계 근거 포함
- **Phase 3**: 동종 Stacking / Voting — 단독 GBM과 동률 → GBM 최종 선택
""")

    # 파이프라인 결과가 있으면 실제 수치 사용
    if 'model_meta' in pipe_results:
        meta = pipe_results['model_meta']
        cv_r2   = meta.get('cv5_r2',  0.6751)
        cv_rmse = meta.get('cv5_rmse', 0.0846)
        cv_mae  = meta.get('cv5_mae',  0.0702)
        cv_std  = meta.get('cv5_std',  0.0227)
    else:
        cv_r2, cv_rmse, cv_mae, cv_std = 0.6751, 0.0846, 0.0702, 0.0227

    c1,c2,c3,c4 = st.columns(4)
    c1.metric("5-Fold CV R²",   f"{cv_r2:.4f}", delta=f"+{cv_r2-0.6592:.4f} vs Baseline")
    ...
    with col_ph:
        ...
        ax.axhline(0.67,color='gold',linestyle='--',lw=1.5,label='목표 R²=0.67')
        ax.legend(fontsize=8); plt.tight_layout(); st.pyplot(fig); plt.close()
    ...
    with col_pr:
        ...
        param_rows.append({'파라미터': 'feature_set', '값': 'C (23개)'})
...
# ╚══════════════════════════════════════════════════════════════╝
with tabs[4]:
    sec_hdr("🧠 XAI 유의인자 분석 — 5-Method Voting")
...
| 방법 | 유형 | 특징 | 한계 |
|---|---|---|---|
| **GB Built-in** | Model-based | 분기 정보이득 합산 | 연속형 피처 편향 |
| **RF Importance** | Model-based | 다양한 트리로 안정적 | 상관 피처 중요도 분산 |
| **ET Importance** | Model-based | 완전 랜덤 분기로 노이즈 강건 | 개별 성능 낮음 |
| **Permutation** | Model-agnostic | 실제 예측 기여 직접 측정 | 상관 피처 문제 |
| **Pearson \|r\|** | Statistical | 선형 관계 해석 용이 | 비선형 놓칠 수 있음 |
""")

    with st.spinner("XAI 분석 중... (약 40초)"):
    ...
    with col_x1:
        ...
        rd = top_xai[['feature','label','GB','RF','ET','Permutation','Pearson','Ensemble_Score']].copy()
        ...
        st.dataframe(rd.style.background_gradient(subset=['Ensemble_Score'],cmap='YlOrRd')
                            .format({c:'{:.4f}' for c in
                                     ['GB','RF','ET','Permutation','Pearson','Ensemble_Score']}),
    ...
    mcols = ['GB','RF','ET','Permutation','Pearson']
    hm_d  = xai_df[mcols].values
    fig, ax = plt.subplots(figsize=(10,9))
    im = ax.imshow(hm_d, aspect='auto', cmap='YlOrRd')
    ax.set_xticks(range(5)); ax.set_xticklabels(mcols,fontsize=10,fontweight='bold')
    ...
    ax.set_title('5-Method XAI Voting Matrix',fontsize=12,fontweight='bold')
    plt.tight_layout(); st.pyplot(fig); plt.close()
